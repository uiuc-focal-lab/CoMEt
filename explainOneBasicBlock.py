...
def main():
    ...
    parser.add_argument("cost_model", type=str, choices=['ithemal', 'uica'], help='cost model whose throughput predictions are to be explained')
    ...
    print('='*100)
    if args.verbose:
        print('Basic blocks in which explanation predicates apply and cost model predicts close to the original throughput prediction:')
        print()
        print('\n\n'.join([x[0] for x in exp.examples(only_same_prediction=True)]))
        print('='*100)
        print()
        print('Basic blocks in which explanation predicates apply and cost model predicts far from the original throughput prediction:')
        print()
