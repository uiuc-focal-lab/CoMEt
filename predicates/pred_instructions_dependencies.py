...
class InstructionsDependencies:
    ...
    def make_predicates_inst_dep(self):  # this has forward edges removed, with no differentiation between the data dependencies
    ...
    def perturb_inst_dep(self, present_tokens, n):
        '''
        ...
        # teh keys of present_tokens indicate the instruction numbers of present instructions and the instruction numbers of instructions in a dependency separated by '_' for present dependency
        ...
        for (i, j) in self.data_dependency_graph.edges():
            if np.random.uniform(0, 1) > 0.5 and (i, j) not in present_pred_dep:
        ...
        for i, inst in enumerate(self.instructions):
            if i in present_pred_inst:  # if this instruction must be present
                perturbed_inst = inst.get_original_asm()  # the original asm is reused
                all_changes[i] = np.ones((len(inst.get_tokens())))  # no change happened to this instruction
                ...
                my_inst_changes = 1
                if 0 in changes:  # some change happened in this instruction
                    my_inst_changes = 0
