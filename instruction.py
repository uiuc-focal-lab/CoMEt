...
class Instruction:
    ...
    def perturb(self, present_tokens, p, n, use_stoke=False):
        ...
        if np.random.uniform(0, 1) > p:  # replace instruction with 'nop'
            return '', data
        # else:
        if np.random.uniform(0, 1) > (1-p):  # don't perturb instruction with 50% probability
            original_my_asm = self.disas['asm']
            if self.is_lock:
                original_my_asm = 'lock ' + original_my_asm
        ...
        if use_stoke:
            ...
            # print('stoke work')
            _, stoke_file = tempfile.mkstemp()
            stoke_original_asm = self.get_original_asm()
            # convert to att syntax
            # get hex
            my_hex = get_hex_of_code(stoke_original_asm).decode('utf-8')
            # print('my hex:', my_hex)
            # disassemble it to att
            my_disasm_att = subprocess.check_output(['python3', 'utils/disasm.py', my_hex], universal_newlines=True, stderr=subprocess.DEVNULL).strip()
            my_inst = my_disasm_att.splitlines()[1].strip()
            # print('original:', stoke_original_asm, 'hex:', my_hex, 'att:', my_inst)
            # put in harness
            harness_copy = copy.deepcopy(harness)
            harness_code = harness_copy.replace('CODE', my_inst)
            with open(stoke_file, 'w') as fp:
                fp.write(harness_code)
            # call stoke debug search
            seed = np.random.randint(10000000)
            subprocess.run(['docker', 'cp', stoke_file, f'stoke:/home/stoke/stoke/data/{stoke_file}'])
            stoke_output = subprocess.check_output(['docker', 'exec', 'stoke', '/home/stoke/stoke/bin/stoke', 'debug', 'search', '--target', f'/home/stoke/stoke/data{stoke_file}', '--seed', str(seed)], universal_newlines=True, stderr=subprocess.DEVNULL)
            # if success continue
            success = ('Successful' in stoke_output)
            counter = 0
            # if fail: repeat till max 5 times
            while not success and counter < 10:
                counter += 1
                seed = np.random.randint(10000000) + counter
                stoke_output = subprocess.check_output(['docker', 'exec', 'stoke', '/home/stoke/stoke/bin/stoke', 'debug', 'search', '--target', f'/home/stoke/stoke/data{stoke_file}', '--seed', str(seed)], universal_newlines=True, stderr=subprocess.DEVNULL)
                success = ('Successful' in stoke_output)
            # if final fail, then return the instruction as is
            if not success:
                return stoke_original_asm, np.ones((len(self.token_list)))
            # if success return the asm from stoke_output
            first = stoke_output.find('After')
            second = stoke_output.find('After', first + 1)
            lines = stoke_output.splitlines()
            lines = lines[3:]
            new_lines = [line[first:second].strip() for line in lines if line.strip()]
            new_inst = new_lines[0]
            # back to intel syntax
            try:
                my_hex_output = get_hex_of_code_att(new_inst)
            except:
                print('original:', stoke_original_asm, 'Perturbed:', new_inst, 'stoke output:', stoke_output, 'harness code', harness_code, 'seed', seed)
                exit(1)
            # disassemble it to intel
            my_disasm_intel = subprocess.check_output(['python3', 'utils/disasm.py', my_hex_output, '--output-intel-syntax'], universal_newlines=True, stderr=subprocess.DEVNULL).strip()
            new_inst = my_disasm_intel.splitlines()[1].strip()
            # print('stoke worked on:', stoke_original_asm, 'and got:', new_inst)
            # delete the file from docker container of stoke
            subprocess.run(['docker', 'exec', 'stoke', 'rm', f'/home/stoke/stoke/data{stoke_file}'])
            return new_inst, np.zeros((len(self.token_list)))
