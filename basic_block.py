...
import pred_opcodes
import pred_dependencies
import pred_regs
import pred_instructions_dependencies
import instruction
from dependency_types_pools import *
# np.random.seed(42)

class BasicBlock(pred_tokens.Tokens, pred_instructions.Instructions, pred_opcodes.Opcodes, pred_dependencies.Dependencies, pred_regs.Registers, pred_instructions_dependencies.InstructionsDependencies):
    def __init__(self, code, predicate_type, classifier_func, **kwargs):
        ...
        elif self.predicate_type == 'opcode':  # level 1 predicates
            self.make_predicates, self.perturb = self.make_predicates_opc, self.perturb_opc
        elif self.predicate_type == 'dependency':  # level 2 predicates
            self.make_predicates, self.perturb = self.make_predicates_dep, self.perturb_dep
        elif self.predicate_type == 'register':  # level 3 predicates
            self.make_predicates, self.perturb = self.make_predicates_reg, self.perturb_reg
        elif self.predicate_type == 'instruction_dependency':  # instruction and dependency together
            self.make_predicates, self.perturb = self.make_predicates_inst_dep, self.perturb_inst_dep
