...
ITHEMAL_DIR = 'data/sensitivity_analysis/Ithemal'
UICA_DIR = 'data/sensitivity_analysis/uica'
...
df_hsw = pd.read_csv('data/Ithemal_dataset/sensitivity_dataset.csv')

for dir in directories:
    files = os.listdir(os.path.join(ITHEMAL_DIR, dir))
    if len(files) == 0:  # empty directory
        continue
    if '0.82_' not in dir:
        continue
    original_asm = []
    ...
    for file in files:
        ...
        # print(original_asm[-1])
        actual_tp.append(df_hsw.iloc[block_id[-1]]['hsw-true'])
        ...
        bb = basic_block.BasicBlock('.intel_syntax noprefix; '+my_code+'\n', 'instruction', testing_uica, uica_output_type='bottleneck')
        uica_bottleneck.append(bb.get_original_pred())
    ...
    ithemal_error = [ape(uica_preds[i], ithemal_preds[i]) for i in range(len(actual_tp))]
    # kendalltau_uica_ithemal = [kendalltau(uica_preds[i], ithemal_preds[i]) for i in range(len(actual_tp))]

    df = pd.DataFrame(list(zip(original_asm, block_category, block_id, num_insts, anchors, precision, coverage, my_time, anchors_uica, precision_uica, coverage_uica, similarity_uica_ithemal, similarity_uica_ithemal_containership, num_perturbations, actual_tp, ithemal_preds, ithemal_error, uica_preds, uica_bottleneck)), columns=['Code', 'Category', 'MCA block id', 'Number of instructions', 'Anchors', 'Precision', 'Coverage', 'Time taken (Ithemal)', 'Anchors uiCA', 'Precision uiCA', 'Coverage uiCA', 'Similarity bw uica, ithemal anchors', 'containership similarity bw ithemal, uica anchors', 'Number of perturbations', 'Actual TP', 'Ithemal prediction', 'Ithemal error', 'uiCA prediction', 'uica bottleneck'])
    df.sort_values('MCA block id', inplace=True)
    df.to_csv(f'data/results/paper_results/sensitivity/{dir}.csv', index = False)
