...
def containership_similarity(a, b):  # a contained within b
    return int(set(a).issubset(set(b)))
...
# ithemal_preds
# uica_preds
ITHEMAL_DIR = 'data/120_results_inst_dep/Ithemal'
UICA_DIR = 'data/120_results_inst_dep/uica'
...
for dir in directories:
    for file in os.listdir(os.path.join(ITHEMAL_DIR, dir)):
        ...
        dep_there = True
        predicates_start = my_output_str.find('All predicates to explain with:  [', predicates_end)
        if predicates_start != -1:  # dependency predicates are found
            predicates_start = my_output_str.find('[', predicates_start) + 1
            predicates_end = my_output_str.find(']', predicates_start)
            predicates = my_output_str[predicates_start:predicates_end]
            predicates_list_dep = predicates.split(', ')
            predicates_list.extend([predic[1:-1] for predic in predicates_list_dep])  # to remove the quotation marks
        else:
            dep_there = False
        ...
        anchor_idx = my_output_str.find('Anchor: ', original_idx_end) + 8  # where the instruction anchor starts
        ...
        if dep_there:
            anchor_idx_dep = my_output_str.find('Anchor: ', anchor_idx_end) + 8  # where the dependency anchor starts
            anchor_idx_end_dep = my_output_str.find('\n', anchor_idx_dep)  # where the anchor ends
            anchors_out.extend(my_output_str[anchor_idx_dep:anchor_idx_end_dep].split(' AND '))
        ...
        precision_idx = my_output_str.find('Precision: ', anchor_idx_end) + 11  # where the precision starts for instruction predicates
        precision_idx_end = my_output_str.find('\n', precision_idx)  # where the precision ends
        precision_inst.append(float(my_output_str[precision_idx:precision_idx_end].strip()))
        ...
        coverage_inst.append(float(my_output_str[coverage_idx:coverage_idx_end].strip()))

        if dep_there:
            precision_idx_dep = my_output_str.find('Precision: ', anchor_idx_end_dep) + 11  # where the precision starts for dependency predicates
            precision_idx_end_dep = my_output_str.find('\n', precision_idx_dep)  # where the precision ends
            precision_dep.append(float(my_output_str[precision_idx_dep:precision_idx_end_dep].strip()))

            coverage_idx_dep = my_output_str.find('Coverage: ', precision_idx_end_dep) + 10  # where the coverage starts
            coverage_idx_end_dep = my_output_str.find('\n', coverage_idx_dep)  # where the coverage ends
            coverage_dep.append(float(my_output_str[coverage_idx_dep:coverage_idx_end_dep].strip()))
            # print(coverage_dep)
        else:
            precision_dep.append(-1)
            coverage_dep.append(-1)
            # print(coverage_dep)
        ...
        anchor_idx_uica = my_output_str_uica.find('Anchor: ', original_idx_end) + 8  # where the instruction anchor starts
        anchor_idx_end_uica = my_output_str_uica.find('\n', anchor_idx_uica)  # where the anchor ends
        anchors_out_uica = my_output_str_uica[anchor_idx_uica:anchor_idx_end_uica].split(' AND ')
        if dep_there:
            anchor_idx_dep_uica = my_output_str_uica.find('Anchor: ', anchor_idx_end_uica) + 8  # where the dependency anchor starts
            anchor_idx_end_dep_uica = my_output_str_uica.find('\n', anchor_idx_dep_uica)  # where the anchor ends
            anchors_out_uica.extend(my_output_str_uica[anchor_idx_dep_uica:anchor_idx_end_dep_uica].split(' AND '))
        anchors_out_uica = [anc for anc in anchors_out_uica if anc != '']
        # anchors_out.sort()
        idxs_uica = np.zeros((len(predicates_list)))  # bit map of the anchors
        # print(idxs, anchor)
        for anc in anchors_out_uica:
            # print(anc.strip().split('_', 1)[1][:-1])
            # print(anc)
            # try:
            idxs_uica[predicates_list.index(anc)] = 1
            # except:
            #     print("anc", anc, 'anchor out', anchors_out)
            #     exit(1)
        anchors_uica.append(idxs_uica)

        precision_idx = my_output_str_uica.find('Precision: ', anchor_idx_end_uica) + 11  # where the precision starts for instruction predicates
        precision_idx_end = my_output_str_uica.find('\n', precision_idx)  # where the precision ends
        precision_uica_inst.append(float(my_output_str_uica[precision_idx:precision_idx_end].strip()))
        ...
        coverage_uica_inst.append(float(my_output_str_uica[coverage_idx:coverage_idx_end].strip()))

        if dep_there:
            precision_idx_dep = my_output_str_uica.find('Precision: ', anchor_idx_end_dep_uica) + 11  # where the precision starts for dependency predicates
            precision_idx_end_dep = my_output_str_uica.find('\n', precision_idx_dep)  # where the precision ends
            precision_uica_dep.append(float(my_output_str_uica[precision_idx_dep:precision_idx_end_dep].strip()))

            coverage_idx_dep = my_output_str_uica.find('Coverage: ', precision_idx_end_dep) + 10  # where the coverage starts
            coverage_idx_end_dep = my_output_str_uica.find('\n', coverage_idx_dep)  # where the coverage ends
            coverage_uica_dep.append(float(my_output_str_uica[coverage_idx_dep:coverage_idx_end_dep].strip()))

        else:
            precision_uica_dep.append(-1)
            coverage_uica_dep.append(-1)
...
# print('coverage dep', coverage_dep)
df = pd.DataFrame(list(zip(original_asm, block_category, block_id, num_insts, all_predicates, anchors, precision_inst, coverage_inst, precision_dep, coverage_dep, anchors_uica, precision_uica_inst, coverage_uica_inst, precision_uica_dep, coverage_uica_dep, similarity_uica_ithemal, similarity_uica_ithemal_containership, num_perturbations, actual_tp, ithemal_preds, ithemal_error, uica_preds)), columns=['Code', 'Category', 'MCA block id', 'Number of instructions', 'All predicates', 'Anchors', 'Precision Inst', 'Coverage Inst', 'Precision Dep', 'Coverage Dep', 'Anchors uiCA', 'Precision uiCA Inst', 'Coverage uiCA Inst', 'Precision uiCA Dep', 'Coverage uiCA Dep', 'Similarity bw uica, ithemal anchors', 'containership similarity bw ithemal, uica anchors', 'Number of perturbations', 'Actual TP', 'Ithemal prediction', 'Ithemal error', 'uiCA prediction'])
df.sort_values('MCA block id', inplace=True)
# print(len(coverage_uica_dep))
df.to_csv('data/results/ithemal_instr_dep_predicate_anchor_similarity_separate.csv', index = False)
