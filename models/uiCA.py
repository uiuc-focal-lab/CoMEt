...
import random
random.seed(0)
# sys.path.append('~/Documents/cost_model_exp/code_predicates/')
sys.path.append('.')
...
def latReducedDueToFastPtrChasing(uArchConfig: MicroArchConfig, memAddr: Dict, lastWriteBase: Optional[Instr], lastWriteIndex: Optional[Instr],
   ...
   if 'disp' not in memAddr.keys():  #FIXME: changes by Isha
      memAddr['disp'] = 0
...
def getInstructions(disas, uArchConfig: MicroArchConfig, archData, alignmentOffset, noMicroFusion=False, noMacroFusion=False):
   ...
   for instrD in disas:
      ...
      if any(('STACKPUSH' in r) for r in instrD['regOperands'].values()):
         implicitRSPChange = -pow(2, int(instrD['eosz']))
      isBranchInstr = any(True for n, r in instrD['regOperands'].items() if ('IP' in r) and ('W' in instrD['rw'][n])) # is branch inst if instruction pointer in register operand and being written to
      isSerializingInstr = (instrD['iform'] in ['LFENCE', 'CPUID', 'IRET', 'IRETD', 'RSM', 'INVD', 'INVEPT_GPR64_MEMdq', 'INVLPG_MEMb', 'INVVPID_GPR64_MEMdq',
                                             ...
                                             'MOV_DR_DR_GPR64', 'WBINVD', 'WRMSR'])  # such instructions serialize the execution
...
def uiCA_pred(filename, iacaMarkers=False, raw=False, arch='all', trace=None, graph=None, TPonly=False, simpleFrontEnd=False,noMicroFusion=False, noMacroFusion=False, alignmentOffset='0', my_json=None, depGraph=None, initPolicy='diff'):
   ...
   if not arch in list(MicroArchConfigs) + ['all']:
      print('Unsupported microarchitecture')
      exit(1)
   if not initPolicy in ['diff', 'same', 'stack']:
      print('Unsupported -initPolicy')
      exit(1)

   if arch == 'all':
      if TPonly or trace or graph or depGraph or my_json or (alignmentOffset == 'all'):
         print('Unsupported parameter combination')
         exit(1)
      disasList = [xed.disasFile(filename, chip=MicroArchConfigs[uArch].XEDName, raw=raw, useIACAMarkers=iacaMarkers) for uArch in allMicroArchs]
      uArchConfigsList = [MicroArchConfigs[uArch] for uArch in allMicroArchs]
      with futures.ProcessPoolExecutor() as executor:
         TPList = list(executor.map(runSimulation, disasList, uArchConfigsList, repeat(int(alignmentOffset)), repeat(initPolicy),
                                                   repeat(noMicroFusion), repeat(noMacroFusion), repeat(simpleFrontEnd)))
      TPDict = {}
   ...
   uArchConfig = MicroArchConfigs[arch]
   disas = xed.disasFile(filename, chip=uArchConfig.XEDName, raw=raw, useIACAMarkers=iacaMarkers)
   # print("disas:", disas)
   if alignmentOffset == 'all':
      if TPonly or trace or graph or depGraph or my_json:
         print('Unsupported parameter combination')
         exit(1)
      with futures.ProcessPoolExecutor() as executor:
         TPList = list(executor.map(runSimulation, repeat(disas), repeat(uArchConfig), range(0,64), repeat(initPolicy), repeat(noMicroFusion),
                                                   repeat(noMacroFusion), repeat(simpleFrontEnd)))
      TPDict = {}
      ...
      TP = runSimulation(disas, uArchConfig, int(alignmentOffset), initPolicy, noMicroFusion, noMacroFusion, simpleFrontEnd,
                    not TPonly, trace, graph, depGraph, my_json)
      if TPonly:
         # print('{:.2f}'.format(TP))
         return '{:.2f}'.format(TP)
