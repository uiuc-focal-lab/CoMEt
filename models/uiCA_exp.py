...
def runSimulation(disas, uArchConfig: MicroArchConfig, alignmentOffset, initPolicy, noMicroFusion, noMacroFusion, simpleFrontEnd,
   ...
   exp_map = np.zeros((len(instructions)))  # explanation map for all the instructions
   # In every round
   num_cycles_done = uopsForRelRound[0][lastApplicableInstr][-1].retired  # this is the number of cycles completed before we start the explanation computation
   for my_rnd in range(1, len(uopsForRelRound)):
      # every intruction's retire cycle will be seen
      for idx, inst in enumerate(instructions):  # each instruction either increases the number of cycles or lets it remain the same
         # the instruction's last uop's retire cycle is to be seen
         inst_retire = uopsForRelRound[my_rnd][inst][-1].retired
         if inst_retire > num_cycles_done:  # this is an important instruction
            exp_map[idx] += inst_retire - num_cycles_done
            num_cycles_done = inst_retire
         # difference of the retire cycle over the last cycle before it will be added as attribution to that instruction for runtime

   # the number for all the instructions will be averaged over the number of rounds, as for tp value computation
   exp_map /= (len(uopsForRelRound)-1)
   print("Explanation:", exp_map)
